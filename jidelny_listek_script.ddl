...
CREATE TABLE jidla (
    ...
    cena_bez_dph FLOAT(10) NOT NULL,
    id_menu      INTEGER
);
...
ALTER TABLE polozky ADD CONSTRAINT polozka_pk PRIMARY KEY ( id_objednavka,
                                                            id_jidlo );
...
CREATE TABLE tydny (
    tyden_cislo INTEGER NOT NULL,
    vsedni_den  VARCHAR2(10) NOT NULL
);

ALTER TABLE tydny ADD CONSTRAINT tyden_pk PRIMARY KEY ( tyden_cislo );
...
ALTER TABLE menu
    ADD CONSTRAINT menu_tyden_fk FOREIGN KEY ( tyden_cislo )
        REFERENCES tydny ( tyden_cislo );
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'st' ) THEN
        raise_application_error(-20223, 'FK STUDENT_STRAVNIK_FK in Table STUDENTI violates Arc constraint on Table STRAVNICI - discriminator column typ_stravnik doesn''t have value ''st'''
        );
    END IF;
